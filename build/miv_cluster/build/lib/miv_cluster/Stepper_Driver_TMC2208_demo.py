...
class Stepper_Driver(object):
	...
	def __init__(self, pinStep, pinDir, pinEn, pulsesPerStep, stepsPerRev, accel0, accelMax, accelMin, speed0, speedMax, speedMin, dir = POSITIVE):
		...
		self.setPulseSpeedTarget(speed0)
		self.setPulseAcceleration(accel0)
	...
	def setPulseSpeedTarget(self, speed):
		...
		if (self._pulseSpeedTarget != speed):
			...
			if (self._pulseAccelCount > 0):
				self._pulseAccelCount = int((self._pulseSpeed * self._pulseSpeed) / (2.0 * self._pulseAcceleration)) # Equation 16
	...
	def runToPulseCount(self, pulses):
		...
		if(self._movementMode == RELATIVE):
			self._pulseCountAbsTarget = self._pulseCountAbs + pulses
		else:
			self._pulseCountAbsTarget = pulses
	...
	def computeNewSpeed(self):
		...
		pulsesToStop = round((self._pulseSpeed * self._pulseSpeed) / (2.0 * self._pulseAcceleration)) # Equation 16
		...
		if (self._pulseAccelCount == 0):
			...
			# set direction to start moving http://www.airspayce.com/mikem/arduino/AccelStepper/AccelStepper-1.61n
			if(pulsesToGo < 0):
...
##############################
	...
	def runToPulseSpeed(self, pulseSpeed):
		...
		self.convergeSpeed()
		while (not (self._pulseSpeedTarget == 0 and self._pulseSpeed == 0) and not self._stopped and self._powered and self._enabled): #returns false when target position is reached
			if (self.runSpeed()): #returns true, when a step is made
				self.convergeSpeed()
			pass
		self._active = False
	...
	def convergeSpeed(self):
		...
		if (self._pulseSpeed > self._pulseSpeedTarget):
			self._pulseAccelCount = self._pulseAccelCount - 1
		...
		if (self._pulseAccelCount == 0):
			...
			if (self.pulsesToGo() < 0):
				self.setDirection(NEGATIVE)
			elif (self.pulsesToGo() > 0):
				self.setDirection(POSITIVE)
		# not first pulse from stoped
		else:
			self._pulsePeriod = self._pulsePeriod - ((2.0 * self._pulsePeriod) / ((4.0 * self._pulseAccelCount) + 1)) # Equation 13
