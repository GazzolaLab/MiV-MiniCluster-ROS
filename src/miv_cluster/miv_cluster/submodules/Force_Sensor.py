...
# DEFINE FORCE SENSOR CLASS FOR GANTRY SERVICE
class Force_Sensor:
    def __init__(self, pin_data, pin_sck, motor):
        self.motor = motor
        self.driver = motor.driver
        self.alert = None
        referenceUnit = 208000                           # measured 208000 at 1kg
        self.threshold = 2.5                             # 2.5kg 
        # load cell sometimes gives unreal large value causing the robot to stop
        # add ceiling to eliminate the effect
        self.ceiling = 5     
        self.checkFreq = 200
        ...
        self.hx.reset()
        self.hx.tare()                                   # zeroing
        offset = self.hx.get_offset()
        print("Tare done! Offset is ",offset)
    
    def measure_force_in_loop(self):
        while self.driver.getActive():
            val = self.hx.get_weight(1)              #unit in kg, up direction is positive
            print(val)         
            if self.threshold < val < self.ceiling: 
                self.stop_motor()
                self.motor.pos_StoppedbyForceSensor = self.driver.getRevCountAbs()*self.motor.revToUnit
    ...
    def measure_force_single(self, times):
        val = self.hx.get_weight(times)              #measure several times and take median
        return val
...
# adopt from http://github.com/tatobari/hx711py
class HX711:
